...
Promise.resolve().then(() => {
  ...
  //! 2) Find all performer docs
   ...
   //- https://docs.mongodb.com/manual/reference/operator/query/regex/
      return Movie.find({ title: { $regex: /C/, $options: 'i' }}, function(err, movTitle) {
         movTitle
      });
}).then((result) => {
   ...
   //- https://docs.mongodb.com/manual/reference/operator/query/regex/
      return Performer.find({ name: { $regex: /J/, $options: 'i' }}, function(err, perfName) {
         perfName
      });
}).then((result) => {
   ...
   //!     console.log the updated movie.
      return Promise.all([ Movie.findOne({ title: 'Caddyshack' }), Performer.findOne({ name: 'Bill Murray' }) ]);
}).then((result) => {
